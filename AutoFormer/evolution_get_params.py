...
class EvolutionSearcher(object):
    ...
    def is_legal(self, cand):
        ...
        # eval_stats,_ = evaluate(self.val_loader, self.model, self.device, amp=self.args.amp, mode='retrain', retrain_config=sampled_config)
        ...
        info['visited'] = True
        info['net_setting'] = sampled_config
    ...
    def compute_meco(self):
        ...
        with torch.no_grad():
            ...
            while len(self.candidates) < 1000:
                try:
                    cand = next(cand_iter)
                    self.model.eval()
                    infor = self.is_legal(cand)
                        # continue
                    self.candidates.append(cand)
                    # can_ = self.get_random_cand()
                    # print(type(can_))
                
                    try:
                        config = infor['net_setting']
                    except:
                        continue
                    # model_module = unwrap_model(self.model_org)
                    # model_module.set_sample_config(config=config)
                    # self.model.train()
                    # self.model.to(self.device)
                    # model.to(self.device)
                    # measures, meco_layers,eigen_layers,layer_shape_C,zero_percent_laeyers = get_score_Meco_result_ident_zero(self.model, x, y, self.device,1)
                    # del self.model
                    # torch.cuda.empty_cache()
                    # mecos.append(measures)
                    accs.append(infor['test_acc'])
                    # print(infor)
                    # print("mecoooo00000000000: ",measures)
                    infor_mecos = {}
                    infor_mecos['net_setting'] = infor['net_setting']
                    ...
                    infor_mecos['test-accuracy'] = infor['test_acc']
                    # infor_mecos['zero_percent_laeyers'] = zero_percent_laeyers
        ...
        with open('./autoFM_1k.json', "w") as file:  # Open file in binary write mode
...
def main(args):
    ...
    dataset_val, args.nb_classes = build_dataset(is_train=False, args=args, folder_name="train")
    ...
    data_loader_test = torch.utils.data.DataLoader(
        dataset_test, batch_size=int(256),
    ...
    # searcher.compute_meco()

 
    api_ = '/home/tuanvovan/MeCo/Meco_explore/Cream/AutoFormer/AutoFM_CVPR2022_API_5_7M.json'
    f = open(api_)
    ...
    with torch.no_grad():
        for k,v in api_1k.items():
            ...
            n_parameters = model_without_ddp.get_sampled_params_numel(v['net_setting'])
            n_parameters = n_parameters / 10.**6
            # n_parameters = sum(p.numel() for p in model_module.parameters() if p.requires_grad)
            print('number of params:', n_parameters)
            # measures, meco_layers,eigen_layers,layer_shape_C,zero_percent_laeyers = get_score_Meco_result_ident_zero(model_module, x, y, device,1)
            # del model_module
            torch.cuda.empty_cache()
            print('________',i)
            # print(measures)
            print('acc: ', v['test-accuracy'])
            # mecos.append(measures)
            # accs.append(v['test-accuracy'])
            # print(infor)
            # print("mecoooo00000000000: ",measures)
            infor_mecos = {}
            ...
            infor_mecos['params'] = n_parameters
            # infor_mecos['zero_percent_laeyers'] = zero_percent_laeyers
            list_infor_mecos[i] = infor_mecos
            i+=1
            # del measures
            ...
            #         json.dump(list_infor_mecos, file)
    with open('./autoFM_CVPR_5_7M_0_1000_params_seed0.json', "w") as file:  # Open file in binary write mode
        json.dump(list_infor_mecos, file)
    spearman_corr = stats.spearmanr(mecos,accs)
