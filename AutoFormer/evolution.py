...
class EvolutionSearcher(object):
    ...
    def get_api(self):
        ...
        with torch.no_grad():
            ...
            while len(self.candidates) <= args.num_net:
                try:
                    cand = next(cand_iter)
                    self.model.eval()
                    ...
                    API_infor_mecos[i] = infor_mecos
                    i+=1
                except:
                    continue
        
        with open(args.path_save_api, "w") as file:  # Open file in binary write mode
            json.dump(API_infor_mecos, file)
...
def get_args_parser():
    ...
    parser.add_argument('--path-save-api', default='./AutoFM_AAAI24_API.json')
    parser.add_argument('--num-net', default=1000, type=int)
