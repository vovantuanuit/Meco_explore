...
meco_scores = []
test_acc=[]
f = open(net_save_path)
net_datas = json.load(f)
torch.manual_seed(42)
torch.backends.cudnn.deterministic = True
torch.backends.cudnn.benchmark = False
for i,v in net_datas.items():
    st = time.time()
    print(i)
    setting = v['net_setting']
    # setting = ofa_network.sample_active_subnet()
    ofa_network.set_active_subnet(ks=v['net_setting']['ks'],e=v['net_setting']['e'],d=v['net_setting']['d'])
    subnet = ofa_network.get_active_subnet(preserve_weight=False)
    ...
    subnet.train()
    subnet.to(device)
    measures, meco_layers,eigen_layers,layer_shape_C,zero_percent_laeyers = get_score_Meco_result_ident_zero(subnet, x, y, device,1)
    ...
    top1 = v['test-accuracy']
    print('top-1 Acc: ',top1)
    del subnet
    infor_mecos = {}
    infor_mecos['net_setting'] = setting
    ...
    # infor_mecos['identical_percent_layers'] = identical_percent_layers
