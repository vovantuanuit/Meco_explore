...
class Searcher(object):
    ...
    def __init__(self, args, device, model, model_without_ddp, choices, train_loader, val_loader, test_loader, output_dir,api):
        self.device = device
    ...
    def get_dss_score(self):

        
        

        api_1k =json.load(self.api)
        dss_scores = []
        accs = []
        list_infor_mecos = {}
        for k,v in api_1k.items():
            ...
            dss_scores.append(indicators['AutoProxA'])  # [Xinda] change dss to AutoProxA
            accs.append(v['test-accuracy'])
            # print(infor)
            # print("mecoooo00000000000: ",measures)
            infor_mecos = {}
            infor_mecos['net_setting'] = v['net_setting']
            infor_mecos['dss'] = indicators['AutoProxA']  # [Xinda] change dss to AutoProxA
            infor_mecos['test-accuracy'] = v['test-accuracy']
            list_infor_mecos[k] = infor_mecos
        with open(args.save_result_json, "w") as file:  # Open file in binary write mode
            json.dump(list_infor_mecos, file)

        spearman_corr = stats.spearmanr(dss_sco res,accs)
...
def get_args_parser():
    ...
    parser.add_argument('--save-result-json', default='')
    # search parameters
    parser.add_argument('--indicator-name', default='AutoProxA', type=str)  # [Xinda] change dss to AutoProxA
    parser.add_argument('--max-epochs', type=int, default=1)
    parser.add_argument('--population-num', type=int, default=1000)
...
def main(args):
    ...
    t = time.time()
    api = open(args.api)
    searcher = Searcher(args, device, model, model_without_ddp, choices, data_loader_train, data_loader_val, data_loader_test, args.output_dir,api)

    searcher.get_dss_score()
